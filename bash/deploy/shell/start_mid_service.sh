...
#===============================================================================
#
#          FILE:  start_mid_service.sh
#
#         USAGE:  ./start_mid_service.sh
#
#   DESCRIPTION:  start middleware service
...
#3# check port list
# List of ports that need to be used!

#./conf/service_port_list, record service and port, Column correspondence
port_list=($(cat ./conf/service_port_list | awk 'NR==1 {print}'))
[ ! -z $port_list ] || port_list=(6379 9200 2181 9092 9600 5601)

service_list=($(cat ./conf/service_port_list | awk 'NR==2 {print}'))
[ ! -z $service_list ] || service_list=(redis elasticsearch zookeeper kafka logstash kibana)
column_num=$(cat ./conf/service_port_list | awk '{print NF}' | awk 'NR==1 {print}')
#port:service number
[ ! -z $column_num ] || $column_num='6'

echo -e "\n\033[40;32mCheck valid server:port: \033[0m"

#Determine if it matches: port:service
if [ ! -f ./conf/service_port_list ]; then
	col_num_1=$(cat ./conf/service_port_list | awk '{print NF}' | awk 'NR==1 {print}')
	col_num_2=$(cat ./conf/service_port_list | awk '{print NF}' | awk 'NR==2 {print}')
	if [ $col_num_1 != $col_num_2 ]; then
		echo -e "\tPlease check: \033[40;34m ./conf/service_port_list configuration \033[0m"
		exit 1
	fi
fi
...
do
	...
	if [ 0 -ne $? ]; then
		## traversal service list
		for j in "${service_list[@]}"
		do
			## compare port service
			for ((n=1;n<=$column_num;n++))
			do
				export n
				port_compare=$(cat ./conf/service_port_list | awk '{print $ENVIRON["n"]}' | awk 'NR==1 {print}')
				service_compare=$(cat ./conf/service_port_list | awk '{print $ENVIRON["n"]}' | awk 'NR==2 {print}')

				[ $i -eq $port_compare -a $j == $service_compare ] && echo -e "\t\033[40;32m$j:$i\033[0m is valid!"
			done
	...
	do
		...
		do
			...
			[ $i -eq $port_compare -a $k == $service_compare ] && echo -e "\t\033[40;33m$k:$i\033[0m is already used!" 
		done
...
pid_list=$(for i in "${port_list[@]}"; do lsof -i :$i; done | awk '{print $2}' | grep -v PID | uniq)
echo $pid_list | grep  '[[:alpha:]]' 1>/dev/null 2>&1
if [  1 -eq $? -a -n "$pid_list" ]; then
	echo -e "\n\033[40;31mWhether to kill the service process being used!\033[0m"
	read -p "kill -9 [y/n]: " kill_var

	if [ x$kill_var == x"y" ]; then
		kill -9 $pid_list
		[ 0 -eq $? ] && echo -e "\t\033[40;31mkill success!\033[0m" || exit 1
		#if [ 0 -eq $? ]; then
		#	ppid_list=$(echo $pid_list | xargs)
		#	echo -e "\t\033[40;31m'kill -9 $ppid_list' success!\033[0m"
		#fi
	elif [ x$kill_var == x"n" ]; then
		echo -e "\t\033[40;31mYou cannot continue to install the service!\033[0m"
		exit 1
	else
		echo -e "\t\033[41;36mplease enter [y/n]\033[0m"
		exit 1
	fi
fi
