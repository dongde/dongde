...
#                   Logstash		9600    dcp-log.conf	/opt/logstash*
#       OPTIONS:  ---
...
unset i
i=$(find $work_dir/ -name "*logstash*tar*" -type f)

if [ "$i" ]; then
	[ "$(echo $i | grep zip)" ] && unzip $i -d $install_dir 1>/dev/null 2>&1 || tar xf $i -C $install_dir
	logstash_dir=$(find $install_dir/logstash* -maxdepth 0 -type d)
	sed  "s/localhost/$local_host_ip/" $work_dir/conf/dcp-log.conf > $logstash_dir/config/dcp-log.conf
	echo -e "\tinstall logstash success!\n"
else
